...
const ScrollingDropdown: FC<Props> = ({ data, disabled, onChange }: Props) => {
    const [shouldShowOptions, setShouldShowOptions] = useState(false);
    const [menuWidth, setMenuWidth] = useState('auto'); // Initialize with 'auto' width
    ...
    // Calculate and set the menu width when shouldShowOptions changes
    useEffect(() => {
        if (shouldShowOptions) {
            const containerWidth = containerRef.current.clientWidth;
            setMenuWidth(containerWidth + 'px');
        }
    }, [shouldShowOptions]);
    ...
    return (
        ...
        >
            ...
            {shouldShowOptions && (
                <div
                    ref={optionsRef}
                    className="absolute bg-custom-background border border-custom-light-blue-50 border-b-0 text-xs max-h-40 overflow-y-scroll"
                    style={{
                        bottom: `${containerRef.current.clientHeight}px`,
                        width: menuWidth,
                    }}
                >
                    {data.map((d, index) => {
